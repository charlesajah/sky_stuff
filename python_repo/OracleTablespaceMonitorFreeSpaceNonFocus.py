...
class OracleTablespaceMonitorFreeSpaceNonFocus(OracleMonitor):
    ...
    def process_database(self, db):
        ...
        # Before calling check_arch_filesystem,first.we create HP_DIAG objects required for monitoring archive_log filesystem.
        #Objects are created only if they do not already exist in the target database.
            ...
            for row in cursor:
                ...
                if not db.is_tablespace_excluded(tablespace):
                    ...
                    if float(pct_free) < check_value:
                        ...
                        if self.db_name in ('TCC011N', 'TCC021N'):
    ...
    def process_datafile_mpoints(self, tspace_name):
        ...
        for mpoint_key, data in self.tablespace_data.items():
            ...
            max_allowable_space = data['available_space'] 
            if max_allowable_space > self.MAX_FILE_SIZE_MB and self.remaining_space_needed > 0:
        ...
        if self.remaining_space_needed > 0:
            logger.info(f"Unable to allocate the required {self.additional_space_needed_mb} MB probably because mountpoints are over 95% full; {self.remaining_space_needed} MB still needed.")
        else:
    ...
    def add_datafiles(self, db, tablespace, free, pct_free, check_value, tspace_size):
        ...
        if cnt == 0:
            ...
            for row in cursor:
                ...
                max_allowable_space = mpoint_avail

                # Check if max_allowable_space is big enough for a full-sized datafile
                if max_allowable_space > self.MAX_FILE_SIZE_MB:

                    logger.info(f"Max allowable space for mpoint {mpoint} is {max_allowable_space}M")
                            ...
                            for key, entry1 in mpoints_big_enough.items():                                
                                ...
                                while self.remaining_space_needed > 0:
                                    ...
                                    if self.MAX_FILE_SIZE_MB < max_allowable_space :
                                        ...
                                        #size_mb=math.ceil(max_allowable_space)
                                        #space_to_allocate=size_mb
                                        #we want only to create datafiles when it can take a full size MAX_FILE_SIZE_MB
                                        break
    ...
    def add_asm_datafiles(self, db, tablespace, free, pct_free, check_value, tspace_size):
        try:
            ...
            logger.exception(f"add_asm_datafiles cursor error: {str(e)}")
        sql=(
        ...
        for row in cursor:
            ...
            #i have replaced the division operator with the floor operator because it rounds it up to nearest whole number. Don't want floats
            if dg_type == 'EXTERN':
                dg_size = dg_size // 1
                dg_free = dg_free // 1
            elif dg_type == 'NORMAL':
                ...
                dg_size = 0  #not sure how to handle disk groups of type FLEX or EXTEND yet
                dg_usable_space = 0 #reason because i want max_allowable_space when computed to equal zero so i can isolate this type of diskgroup.

            if dg_usable_space > 0: #If USABLE_FILE_MB becomes negative, you may not be able to create new ASM files
                ...
                max_allowable_space = dg_size * 0.95 - (dg_size - dg_usable_space) #logic to do away with 95% utilisation cap
                    ...
                    if self.additional_space_needed_mb <= max_allowable_space:
                        ...
                        while self.remaining_space_needed > 0:
                            ...
                            if self.create_asm_data_file(sql_command, tablespace):                                       
                                # Update the remaining space needed
                                self.remaining_space_needed -= size_mb
                elif max_allowable_space == 0:
                    logger.warning(f"diskgroup {dg_name} is probably of type FLEX or EXTEND")
                    return
                else: #Any value less than zero i.e. negative, indicates an existing utilisation of diskgroup beyond 95% 
                    logger.info(f"Diskgroup utilisation is beyond 95%")
                    return

            else:
                logger.warning(f"Urgent! Please add additional ASM disk(s) to diskgroup {dg_name}")
                return
    ...
    def check_arch_filesystem(self, db):
        ...
        try:
            ...
            # check if ASM is configured for the database
            cursor.execute("""select count(*) from V$ASM_DISKGROUP""")            
            asm_check = cursor.fetchone()  # Fetch the result
            cnt = asm_check[0]
            if cnt == 0: #we only want the check_arch_filesystem processed against non-ASM instances.
                ...
                for result in results:
                    ...
                    if not line:
                        continue  # Skip if line is None or empty

                    if 'Checking Database ' in line:
                        db_name= line.split("'")[1] #we split the string usring a single quote and we pick out the Db name which should be the second string.
                        dname = db.get_name()                    
                        # Convert both names to uppercase for case-insensitive comparison
                        db_name = db_name.upper()
                        dname = dname.upper()
                        # Check if dname exactly matches db_name
                        self.exact_match = dname.strip() == db_name.strip() #this will store a boolean TRUE or FALSE.
                        
                        # Check if dname is contained within db_name and if it's preceded by 'C' for container DBs
                        # db_name.find(dname) > 0 ensures the first element is not the string being searched for which allows us to match something like JBP011N in CJBP011N
                        self.contained_match = (dname in db_name and db_name.find(dname) > 0 and db_name[db_name.find(dname) - 1] == 'C') or (db_name in dname)
                    if self.exact_match or self.contained_match: #checking if either of them returned True
                        dname = db.get_name() 
                        self.dname = dname.upper() #we store the database name in a global variable
                        if 'Checking Directory ' in line:     
                                parts = line.split("'")  # Split by single quotes to capture the ASM diskgroup   
                                dir_line = parts[1]                   
                                if dir_line.startswith('+'):  # ASM paths typically start with a '+', so this handles ASM storage                               
                                    if len(parts) > 1:  # Ensure there's a part within quotes
                                        dir_line = parts[1]  # This should capture the ASM diskgroup path
                                        util_val = parts[2]  # We extract the % utilisation for the ASM path
                                        percent_util = util_val.split()[-3].strip('%')
                                        #db_name = dname  #when container DBs are involved just use the pdb name
                                ...
                                if  float(percent_util) > 90.0 : 
                                    logger.info(f"Collecting Archivelog data ...")
                                    logger.info(f"_datapoint:ArchiveLogFilesystemSpace:{self.dname} ArchiveLogLocation:{dir_line} %Utilisation:{percent_util}%")
                                                    
        except oracledb.DatabaseError as e:
            error, = e.args
            # Check for ORA-29283 specific error
            if 'ORA-29283' in error.message:
                logger.error(f"check_arch_filesystem : ORA-29283 flagged for database  {db.get_name()} - Please ignore!")
            elif 'ORA-29284' in error.message:
                if self.db_name in ('DMS021N', 'INT021N'):
                    logger.error(f"check_arch_filesystem : ORA-29284 flagged for database  {db.get_name()} - UTL_FILE reading error from  SunOS files containing certain characters.")
                else:
                    logger.error(f"check_arch_filesystem : ORA-29284 flagged for database  {db.get_name()}.")
            else:
                logger.error(f" check_arch_filesystem Oracle DB error for database {self.db_name}: {error.message}")

        except Exception as e:
            logger.exception(f"check_arch_filesystem error occurred for database {self.dname}: {error.message}")
    ...
    def run(self):
        ...
        if self.check_args(args):
            if self.populate():
                ...
                #transcomm DBs experience long delays during the addition of datafiles 
                #when this is aborted by the build the rest of the DBs in the list are not processed
                #This logic below is allow the processing of Transcomm DBs last after all DBs must have been processed.
                transcom_dbs = {"TCC011N", "TCC021N"}
                other_dbs = [db for db in self.databases if db.get_name() not in transcom_dbs]
                delayed_dbs = [db for db in self.databases if db.get_name() in transcom_dbs]
                all_dbs = other_dbs + delayed_dbs
                for db in all_dbs:  # Loop through the databases in the order of other_dbs first and then delayed_dbs       
