...
# Convert the lag times from 'HH24:MI:SS' format to seconds
...
def check_vital_signs(instance_name, host, port, user, password):
    try:
        ...
        if result3 is not None:
            ...
            write_lag_seconds = convert_to_seconds(write_lag)
            flush_lag_seconds = convert_to_seconds(flush_lag)
            replay_lag_seconds = convert_to_seconds(replay_lag)
            if write_lag_seconds > 0 or flush_lag_seconds > 0 or replay_lag_seconds > 0 :                
                logger.info("")
        ...
        query5="""select pid, to_char(date_trunc('second',now() - pg_stat_activity.query_start), 'HH24:MI:SS') AS duration, state, datname FROM pg_stat_activity WHERE state='active' and usename!='repmgr' and  (now() - pg_stat_activity.query_start) > interval '5 minutes';"""
