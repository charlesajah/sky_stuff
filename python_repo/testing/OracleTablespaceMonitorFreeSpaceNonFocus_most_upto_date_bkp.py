...
class OracleTablespaceMonitorFreeSpaceNonFocus(OracleMonitor):
    ...
    def process_datafile_mpoints(self, mpoint, mpoint_avail, tspace_name, max_allowable_space):
        ...
        try:
            cursor = self.connection.cursor()
            cursor.arraysize = 1000
            #well this is me assuming any filesystem with 'data' in the mount point name must be a dedicated filesystem for datafiles?
            #will ask to verify
            cursor.execute("""select fsize,used,avail,use_pcent,mounted_on from dev.filesystem where mounted_on like '%/ora/data%'""") 
        except Exception as error:
            print(f"Querying the External Table failed: {str(error)}") 
            return
        
        for row in cursor:
            mp_size = self.convert_to_megabytes(row[0]) # Convert to megabytes
            mp_used = self.convert_to_megabytes(row[1]) # Convert to megabytes
            mp_avail = self.convert_to_megabytes(row[2])  # Convert to megabytes
            mp_use_pcent = row[3]
            os_mp=row[4]
            ...
            self.tablespace_data[os_mp] = entry2  # Use tablespace name as the key

        # Iterate through mount points to try and fit the remaining space
        for mpoint_data in self.tablespace_data.values():
            if self.remaining_space_needed <= 0:
                break  # Break the loop if no more space is needed

            mpoint = mpoint_data['mountpoint']
            mpoint_avail = mpoint_data['available_space']
            mpoint_size = mpoint_data['mpoint_size']
            ...
            if max_allowable_space >= 0:
                ...
                print(f"Attempting to allocate {space_to_allocate} MB to {mpoint}")
                if space_to_allocate > 0:
                    self.remaining_space_needed -= space_to_allocate
                    self.space_allocations[mpoint] = space_to_allocate
                    print(f"Allocated {space_to_allocate} MB to mount point {mpoint}")
            else:
                print(f"No space available for allocation on {mpoint}. Already at or above 95% utilization.")
                space_to_allocate = 0  # No space should be allocated
        cursor.close()
        
        return self.remaining_space_needed, self.space_allocations
        ...
        for row in cursor:
            ...
            if cnt == 0:
                ...
                for row in cursor:
                    ...
                    #mpoint_used = mpoint_size - mpoint_avail  # Calculate used space in mountpoint

                    # Calculate the maximum consummable space (95% of total space of OS MountPoint)
                    #max_allowable_space = mpoint_size * 0.95
                    ...
                    if max_allowable_space >= 0:
                        ...
                        if total_space_if_added <= max_allowable_space:
                            ...
                            print(f"Required space for {tspace_name} cannot fit into the mount point {mpoint}") 
                            print(f"Total additional space needed: {self.additional_space_needed_mb} MB and available space for mpoint is {mpoint_avail} whilst maximum allowable space for mountpoint {mpoint} is {max_allowable_space} ")

                            try:
                                #ok i thought about putting this SQL query outside the loop but i decided against it
                                #because the filessystem's available space could have been altered 
                                #by any of the preceeding code.
                                #so we want the most accurate and current value
                                cursor.execute("""select fsize,used,avail,use_pcent,mounted_on from dev.filesystem
                                                where mounted_on like '/trcomdbn01/ora/data%'""") 
                                ...
                                for row in cursor:
                                    ...
                                    tablespace_data[os_mp] = entry2  # Use tablespace name as the key
                                    print(f"Dict tablespace_data contains {len(tablespace_data)} records")
                                if len(tablespace_data) > 0:
                                    print(f"Dict tablespace_data contains {len(tablespace_data)} records")
                            ...
                            # Iterate through mount points to try and fit the remaining space
                            for mpoint_data in tablespace_data.values():
                                if self.remaining_space_needed <= 0:
                                    break  # Break the loop if no more space is needed
                        ...
                        print(f"Checking the value of remaining_space_needed {self.remaining_space_needed}")
                        if self.remaining_space_needed > 0:
                            print(f"Unable to allocate the required {self.additional_space_needed_mb} MB; {self.remaining_space_needed} MB still needed.")
                        else:
                            print(f"Successfully allocated {self.additional_space_needed_mb} MB across mount points.")
