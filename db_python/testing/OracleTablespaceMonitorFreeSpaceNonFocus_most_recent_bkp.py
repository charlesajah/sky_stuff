...
class OracleTablespaceMonitorFreeSpaceNonFocus(OracleMonitor):
    ...
    def add_datafiles(self, db, tablespace, pct_free, check_value, tspace_size):
        ...
        entry1 = None  # Declare entry1 outside the if block
        ...
        for row in cursor:
            ...
            if cnt == 0:
                ...
                tablespace_data = {}  # dictionary to store all tablespace OS Mount point data
                    ...
                    if mpoint_avail > additional_space_needed_mb:
                        ...
                        if len(mpoints_big_enough) > 0:
                            for key, entry1 in mpoints_big_enough.items():                                
                                ...
                                # Calculate the maximum allowable space (95% of total space of OS MountPoint)
                                max_allowable_space = mpoint_size * 0.95

                                # Calculate the total space if additional space is added
                                total_space_if_added = mpoint_used + additional_space_needed_mb

                                # Check if total space used will exceed 95% of the mount point after adding the additional space
                                if total_space_if_added <= max_allowable_space:
                                    # Space can be safely added without exceeding 95% of OS filesystem
                                    print(f"Additional space of {additional_space_needed_mb} MB can be added to mount point {mpoint}")
                                    # proceed with adding space logic
                                else:
                                    # Adding space will exceed the 95% threshold
                                    print(f"Cannot add {additional_space_needed_mb} MB to mount point {mpoint} as it exceeds 95% usage limit")
