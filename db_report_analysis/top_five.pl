...
use vars qw($TEST_ID $ENV $GRP $DBNAME $DESC $DEBUG $HELP);
...
SUMMARY
  ...
  -test_id         - First Test ID to compare - format DDMONYY-HH24MI_DDMONYY-HH24MI e.g. 16OCT23-0630_16OCT23-0900 
...
open my $out , '>>' , 'top5_charts_v2.txt' or die "Can't write top5_charts_v2.txt: $!" ;
...
sub doRunReport
{
  ...
  ## Generates file : top5_charts.txt which will include all the Top5 SQLs per snapshot interval per databsse 
  ## Produces all the CVS files into the work area and then copied to the Web Server
  my ($cmd) = "sqlplus -s $connHPDIAG \@top_five_v2.sql $TEST_ID $dbname $g_dirname " ; 
  system("${cmd}");
  print "Done\n";
}
...
## Params      : 
## Description : Copy csv and text files to wd015506 web server.
## *******************************************************************************************************************************
sub doCopyFiles {
    open my $out , '>>' , 'top5_charts_v2.txt' or die "Can't write top5_charts_v2.txt: $!" ;
    print $out "\n" . "h3. Top 5 SQLs Reports\n" ;
    close $out ;
  }
  print "Copying files to http://wd015506.bskyb.com:9320/reports/tests/top5/$g_dirname/ \n" ;
  system ( "mkdir \\\\wd015506.bskyb.com\\d\$\\TelNum_Reports\\appdata\\reports\\tests\\top5\\$g_dirname" ) ;
  system ( "copy /y *.csv \\\\wd015506.bskyb.com\\d\$\\TelNum_Reports\\appdata\\reports\\tests\\top5\\$g_dirname\\." ) ;
  system ( "copy /y *.txt \\\\wd015506.bskyb.com\\d\$\\TelNum_Reports\\appdata\\reports\\tests\\top5\\$g_dirname\\." ) ;
