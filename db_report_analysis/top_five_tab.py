#!/usr/bin/python
###################################################################################
# Name : Top_five_tab.py
# Purpose : Create confluence page (+ child pages) of the top five SQLs between AWR snapshots in a test's duration.
# 
###################################################################################
# initialize
import requests
...
def deletePageByTitle(title):
    ...
    checkPageExistsData = requests.get(url + "?title=" + title + "&expand=history", headers=headers, auth=basic_auth)
    requestJson = checkPageExistsData.json()
    if "results" in requestJson and requestJson["results"]:
        for results in requestJson["results"]:
            pageId = results["id"]
            requests.delete(url + pageId, headers=headers, auth=basic_auth)
...
#
# main block
page_name1 = "Top 5 SQLs between snapshot intervals during a test."
page_name2 = "Top 5 SQLs Table."
#delete old pages
for page_name in [page_name1, page_name2]:
    if pageExists(page_name):
        deletePageByTitle(page_name)


page_id1 = createPage(page_name1, 340554166, 'top_five_tab.txt') 
addlabelstatus = add_labels(page_id1, 'nft_dba')
page_id2 = createPage(page_name2, 340545411, 'top_five_tab.txt') 
addlabelstatus = add_labels(page_id2, 'nft_dba')
