...
SELECT * FROM TABLE ( report_data.Get_test_info ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2 ) ) ;

-- Get brief list of SQL Comparison
SELECT * FROM TABLE ( report_comp.Get_top25_comparison_summary ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2, i_desc => :l_test_desc, i_link => 'Y' , i_mode => 'APP' ) ) ;

-- Get summary for the Top SQL Load Information
select * from TABLE ( report_comp.Get_load_comparison_summary ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2 ) ) ;

-- Wait class comparison summary 
select * from table(report_comp.Get_waitclass_comparison_summary ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2  ));

-- Generate DB summary 
select * from table ( report_data.Get_DB_Summary ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2, i_ratio => 5 , i_filter => 'RED' , i_summary => 'YES' ));
...
SELECT * FROM TABLE ( report_comp.Get_top25_comparison_summary ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2 , i_mode => 'APP' ) ) ;
SELECT * FROM TABLE ( report_comp.Get_top25_comparison_full ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2, i_title => 'N', i_mode => 'APP' ) ) ;
...
SELECT * FROM TABLE ( report_data.Get_billing_rate ( i_testId => :l_test_id1 )) ;
...
SELECT * FROM TABLE ( report_data.Get_Cache_Info ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2 ) ) ;

select 'h3. Chart Comparisons' from dual;
-- Populates the Chart Comparisons between Releases
SELECT * FROM TABLE ( report_comp.Get_chart_comparison ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2 ) ) ;
...
SELECT * FROM TABLE ( report_data.Get_sal_basket_sizes ( i_testId1 => :l_test_id1 ) ) ;

select 'h3. Further Information' from dual;
-- Add the Row Lock Waits Information
SELECT * FROM TABLE ( report_comp.Get_rlw_comparison ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2  )) ;

-- PGA report - this section only output if at least one database has pga growth of 50%+ or 2gb+
SELECT * FROM TABLE ( report_data.Get_PGA_growth ( i_testId => :l_test_id1 ) ) ;
SELECT * FROM TABLE ( report_data.Get_PGA_growth ( i_testId => :l_test_id2 ) ) ;

-- Archived Redo Log report
--select 'h3. Archived Redo Log' from dual;
SELECT * FROM TABLE ( report_data.Get_RedoLogs_Usage ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2 )) ;
...
select * from table( report_comp.Get_top25_detail ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2 , i_mode => 'APP' ));
...
select * from table( report_comp.Get_load_comparison_summary ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2 ));
select * from table( report_comp.Get_load_comparison_full ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2, i_title => 'N' ));
...
select * from table( report_comp.Get_all_db_top10_comparison ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2 , i_mode => 'APP' ));

-- Presents the SQLID changes for the sqm SQL text
select 'h3. SQLID mutations' from dual;
select * from table( report_comp.Get_SQLID_byText ( i_testId1 => :l_test_id1, i_testId2 => :l_test_id2 , i_mode => 'APP' ));
