...
class OracleTablespaceMonitorFreeSpaceNonFocus(OracleMonitor):
                        ...
                        if self.additional_space_needed_mb <= max_allowable_space:
                            ...
                            if len(mpoints_big_enough) > 0:
                                ...
                                while self.remaining_space_needed > 0:
                                    ...
                                    if new_datafile == None:
                                        break
                                    new_datafile=new_datafile.lower()
                                    print("New datafile name:", new_datafile)
    ...
    def adjust_filepath(self, file_path_without_number, tablespace):
        ...
        if last_segment != tablespace:
            file_path_without_number = os.path.join(os.path.dirname(file_path_without_number), tablespace) #if base name is not tablespace name then replace it with tablespace name
            file_path_without_number = file_path_without_number.replace('\\', '/')
            ...
            if cnt != 0: # debugging relic, this one.
                if existing_filenames:
                    ...
                    if match:
                        ...
                        print(f"file_path_without_number in the for loop is: {file_path_without_number}")
            cursor.close()
