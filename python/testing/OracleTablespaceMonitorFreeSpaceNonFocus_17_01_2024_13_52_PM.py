...
class OracleTablespaceMonitorFreeSpaceNonFocus(OracleMonitor):
    ...
    def generate_new_datafile_name(self, tablespace, mount_point, generated_filenames):
        ...
        try:
            ...
            cursor.execute(sql_query, tablespace=tablespace, file_like=f'%{mount_point}%')    
            max_number = 0
            file_path_without_number = None

            for row in cursor:
                file_name = row[0]
                # Extract everything except the numeric suffix
                match = re.match(r"(.+?)(?:_(\d+))?\.dbf$", file_name)
                if match:
                    path_without_number = match.group(1)
                    num = int(match.group(2)) if match.group(2) else 0
                    if file_path_without_number is None or file_path_without_number == path_without_number:
                        file_path_without_number = path_without_number
                        max_number = max(max_number, num)
                    else:
                        raise ValueError("Inconsistent file path patterns found in datafiles.")

            cursor.close()

            if file_path_without_number is None:
                raise ValueError("No suitable datafiles found for the pattern matching.")
            
            new_number = max_number
            
            #Generate a new filename and make sure it is unique
            while True: # While loop is used to keep trying new numbers for the filename until it finds one that hasn't been used yet
                new_number += 1
                new_filename = f"{file_path_without_number}_{str(new_number).zfill(3)}.dbf"
                if new_filename not in self.generated_filenames:
                    break
