...
class OracleTablespaceMonitorFreeSpaceNonFocus(OracleMonitor):
    ...
    def process_datafile_mpoints(self, tspace_name):
        ...
        for mpoint_key, data in self.tablespace_data.items():
            ...
            if max_allowable_space > 0 and self.remaining_space_needed > 0:
                ...
                # Generate new datafile name before allocating space
                new_datafile=self.generate_new_datafile_name(tspace_name, mpoint_key, self.generated_filenames)
                if new_datafile == None:
                    continue
                print("New datafile name:", new_datafile)

                # Now, allocate the space
                size_mb=round(space_to_allocate)
                self.create_data_file(tspace_name, new_datafile, size_mb)
                        ...
                        if self.additional_space_needed_mb <= max_allowable_space:
                            ...
                            if len(mpoints_big_enough) > 0:
                                ...
                                if new_datafile == None:
                                    continue
                                print("New datafile name:", new_datafile)
                                size_mb=round(self.additional_space_needed_mb)
                                self.create_data_file(tablespace, new_datafile, size_mb)
    ...
    def adjust_filepath(self, file_path_without_number, tablespace):
        ...
        if last_segment != tablespace:
            file_path_without_number = os.path.join(os.path.dirname(file_path_without_number), tablespace)
    ...
    def generate_new_datafile_name(self, tablespace, mount_point, generated_filenames):
        ...
        try:
            ...
            cursor.execute(sql_query, file_like=f'%{mount_point}%')    
            max_number = 0
            ...
            if cnt != 0:
                for file_name in existing_filenames:
                    ...
                    if match:
                        path_without_number = match.group(1)
                        num_part = match.group(2)
                        if num_part:
                            num = int(num_part)
                            max_number = max(max_number, num)
                            if file_path_without_number is None:
                                file_path_without_number = path_without_number
            cursor.close()
            ...
            if file_path_without_number is not None:
                ...
                new_filename = f"{file_path_without_number}_{str(new_number).zfill(3)}.dbf"
                #Generate a new filename and make sure it is unique
    ...
    def create_data_file(self, tablespace, new_datafile, size_mb):
        ...
        try:
                ...
                print(f"Establishing cursor for adding datafile to tablespace {tablespace} failed: {str(e)}")
        try:
        ...
        try:
            ...
            print(f"Unable to close cursor for function create_data_file:{str(e)}")
