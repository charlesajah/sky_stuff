...
class OracleTablespaceMonitorFreeSpaceNonFocus(OracleMonitor):
    ...
    def __init__(self):
        ...
        self.MAX_FILE_SIZE_MB = 30000  # Maximum file size in MB
    ...
    def process_datafile_mpoints(self, tspace_name):
        ...
        # we now try to split it up and spread it across multiple mountpoints using this function.
            ...
            if max_allowable_space > 0 and self.remaining_space_needed > 0:
                ...
                while self.remaining_space_needed > 0:
                    ...
                    if new_datafile == None:
                        break
                    print("New datafile name:", new_datafile)
                    size_mb=min(math.ceil(space_to_allocate), self.MAX_FILE_SIZE_MB)
                    print(f"datafile size expected is: {size_mb}")
    ...
    def add_datafiles(self, db, tablespace, pct_free, check_value, tspace_size):
        ...
        for row in cursor:
            ...
            if cnt == 0:
                ...
                #and then we add 20% of the check_value on top of that as well
                self.additional_space_needed_mb = max(0, (tspace_size * (required_min_free_pct / 100) - tspace_size * (pct_free / 100))) + 0.2 * (tspace_size * (required_min_free_pct / 100))
                                ...
                                for key, entry1 in mpoints_big_enough.items():                                
                                    ...
                                    while self.remaining_space_needed > 0:
                                        ...
                                        if self.create_data_file(tablespace, new_datafile, size_mb):
                                            ...
                                            #self.tablespace_data[mount_point]['available_space'] -= space_to_allocate                  
        ...
        if self.remaining_space_needed > 0: #for space requirements greater than a single mount , we use this call to spread it out across mulitiple mpoint.
            ...
            print("Space allocation for the tablespace is complete.")
    ...
    def generate_new_datafile_name(self, tablespace, mount_point, generated_filenames):
        ...
        try:
                ...
                print(f"Establishing cursor for New Datafile name failed: {str(e)}")
        
        try:
            # Execute the SQL query
            cursor.execute(sql_query, file_like=f'%{mount_point}%' , tspace={tablespace})    
            max_number = 0
            file_path_without_number = None
            pattern = re.compile(r"(.+\/)([^\/]+?)(?:_)?(\d+)\.dbf$")
            existing_filenames = [row[0] for row in cursor]
            ...
            if cnt != 0: # debugging relic, this one.
                for file_name in existing_filenames:
                    #file_name=existing_filenames[0]
                    ...
                    if match:
                        ...
                        #print(f"num_part is :{num_part}")
                        file_path_without_number = os.path.join(file_path, base_name)
                print(f"num_part is :{num_part}")
                if num_part and num_part.isdigit():
                    #print(f"file_path_without_number in the for loop is: {file_path_without_number}")# Find the maximum numeric part
                    max_number = max(numeric_parts, default=0)  # Using default=0 for the case when list is empty
                    print(f"max_number ofr tablespace {tablespace}")
            ...
            if file_path_without_number is not None:                           
                ...
                #new_filename=os.path.join(file_path, f"{base_name}_{str(new_number).zfill(3)}.dbf")
                ...
                # Adjust file_path_without_number based on tablespace string in the base name
                file_path_without_number = self.adjust_filepath(file_path_without_number, tablespace)
                new_filename = f"{file_path_without_number}_{str(new_num_part).zfill(4)}.dbf" 
                #check if new filename is unique in the database and has not been generated already in this run
                while new_filename in existing_filenames or new_filename in generated_filenames:
                    new_number += 1
                    new_filename = f"{file_path_without_number}_{str(new_num_part).zfill(4)}.dbf"        
                # Add the new filename to the set and return it
                self.generated_filenames.add(new_filename)
                return new_filename
            else:
                print(f"Unable to match any existing datafile in the mount point {mount_point}")
                return None

        except Exception as e:
              print(f"New Datafile name generation failed:{str(e)}")
              return None
    ...
    def create_data_file(self, tablespace, new_datafile, size_mb):
        ...
        try:
            ...
            print(f"New datafile {new_datafile} successfully added to tablespace {tablespace}")
            cursor.close
            return True
                
        except Exception as e:
            print(f"New Datafile addition to tablespace {tablespace} failed:{str(e)}")
            return False
