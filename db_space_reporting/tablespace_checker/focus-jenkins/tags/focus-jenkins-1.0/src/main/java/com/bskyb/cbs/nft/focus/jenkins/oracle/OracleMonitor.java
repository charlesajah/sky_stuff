...
public abstract class OracleMonitor 
{
			...
			try {
				...
				while (line != null) {
					if ((! line.contains("#") && line.contains(","))) {
						...
						databases.add(new OracleDB(fields[0],fields[1],fields[2],fields[3],fields[4]));
					}
					line = br.readLine();
				}
			} finally {
		...
		try
		{
			...
			for(int i = 0; i < dbArray.length(); i++)
			{
				...
				if((db.has("tns")) && (db.has("sid")))
				{
					...
					databases.add(new OracleDB(sid, tns, "HP_DIAG", "HP_DIAG_1234", "UNDOTBS"));
				}
			}
		}
	...
	protected abstract void processDatabase(OracleDB db);
}
