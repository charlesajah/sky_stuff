#!/usr/bin/python
###################################################################################
# Name : changed_objects.py
# Purpose : Create confluence page for changed objects over 24 hours.
#     Called by jenkins job https://ppejenkins.bskyb.com/view/NFT%20DBA/view/NFT%20DBA%20-%20Miscellaneous%20Jobs/job/NFT%20DBA%20-%20Dataprov%20Reporting/job/NFT%20DBA%20-%20Dataprov%20Data%20Pool%20Build%20Hierarchy%20(N01)/configure
...
# Change History :
#     30-AUG-2024 Stuart Mason initial version
###################################################################################
# initialize
import requests
import json
...
###################################################################################
# main block
page_name = " ".join(args.ENV.split())  # strip out extra space characters (if any)
if page_name == '':
    page_name = '{:%Y%m%d%H%M%S}'.format(datetime.datetime.now())
elif pageExists(page_name) :
    deletePageByTitle(page_name)
page_id = createPage(page_name, 338892856, 'changed_objects.txt')  # 338892856 is 'Daily New and Changed Objects' page.
