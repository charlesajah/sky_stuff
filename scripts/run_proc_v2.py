...
class RunProcedure(Datapump):
    ...
    def run_procedure(self, db, datapump_type):
        try:
            ...
            v_sql =""" 
                ...
                Procedure imp_stg_table(t_name in varchar2) IS 
                   ...
                   EXECUTE IMMEDIATE '
                    ...
                    -- Close the file
                    UTL_FILE.FCLOSE(file_handler);
                    EXCEPTION WHEN NO_DATA_FOUND THEN IF stg_name = 'STAGE_PLAN' THEN dbms_output.put_line(
                    'There is no SQL PLAN baseline dumpfile to import for database ' || name
                    );
                    ELSIF stg_name = 'STAGE_PROF' THEN dbms_output.put_line(
                    'There is no SQL Profile dumpfile to import for database ' || name
                    );
                    END IF;
                end imp_stg_table;
                ...
                Procedure import_profiles IS
                    ...
                    IF cnt = 0 then dbms_output.put_line(
                        'No SQL profile has been imported! for database ' || name);
                        dbms_output.put_line('');
                    END IF;
                EXCEPTION WHEN OTHERS THEN manage_errors();
                RAISE;
                ...
                Procedure import_plan_baselines IS 
                    ...
                    'No SQL plan has been imported for database ' || name
                    );
            ...
            logger.exception(f"Error in executing SQl script: {str(e)}")
