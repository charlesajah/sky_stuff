...
BEGIN
   logger.write ( 'begin' ) ;
   execute immediate 'truncate table ' || l_pool ;
   INSERT /*+ append */ INTO actmobpiggybank t ( t.accountNumber , t.partyId , t.serviceInstanceId, t.messotoken )
   SELECT act.accountNumber , act.partyId , act.serviceInstanceId, act.messotoken
    ...
    WHERE da.balance <= 0  -- no outstanding balance
      ...
      AND act.accountNumber IN (select  account_no from igor.stg_igor_active_accs@IGR011N 
   ...
   SELECT ROWNUM AS pool_seqno , l_pool AS pool_name , s.accountNumber , s.partyId , s.id , s.messotoken
     FROM (
           SELECT d.accountNumber , d.partyId , d.serviceInstanceId AS id, d.messotoken
             FROM actmobpiggybank d
            ORDER BY dbms_random.value
          ) s
    WHERE ROWNUM <= 100000
   ;
