...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Procedure DP_GETCUSTOMER_SNSSERVICES
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "DATAPROV"."DP_GETCUSTOMER_SNSSERVICES" 
...
where bba.portfolioid = bpp.portfolioid
   ...
   and bpp.catalogueproductid in (12675, 13661, 13744, 12721)
   and bsi.serviceinstancetype in (100, 400)
   and BCA.Effectivetodate is null
   and bba.accountnumber = p_accountnumber);
end dp_getcustomer_snsservices;

/

  GRANT EXECUTE ON "DATAPROV"."DP_GETCUSTOMER_SNSSERVICES" TO "BATCHPROCESS_USER";
