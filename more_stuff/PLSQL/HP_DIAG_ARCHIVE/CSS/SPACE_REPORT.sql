...
BEGIN
  ...
  loop
    ...
    PIPE ROW ( '{chart:dataOrientation=vertical|rangeAxisLowerBound=0|dateFormat=dd/mm|height=500|orientation=vertical|stacked=true|timePeriod=Day|timeSeries=true|title=' || r1.db_name || '|type=area|width=1800|xLabel=Date|yLabel=Space (GB)}' ) ;
    PIPE ROW ( ' || DATE_TIME || USED_GB || FREE_GB ||' ) ;
    for r2 in (select '|' || to_char(run_date, 'dd/mm') || '|' || used_gb || '|' || free_gb || '|'  as chart_row
                 from hp_diag.v_db_summary where db_name = r1.db_name
               order by run_date)
    loop
...
BEGIN
  ...
  dbms_output.put_line('{chart:dataOrientation=vertical|rangeAxisLowerBound=0|dateFormat=dd/mm|height=500|orientation=vertical|stacked=true|timePeriod=Day|timeSeries=true|title=' || upper(p_db_name) || '|type=area|width=1800|xLabel=Date|yLabel=Space (GB)}' ) ;
  WITH q AS (
           select * 
  ...
  if length(l_head) > 17 then 
    ...
    if l_cnt = 0 then
      l_sql := 'create table seg_space_info as with q as ( SELECT to_char(run_date,''dd/mm'') run_date, schema_name, max(used_gb) used_gb FROM hp_diag.nft_seg_Space where db_name = ''' || upper(p_db_name) || ''' group by run_date, schema_name order by run_date) select * from  q pivot (max(used_gb) for (schema_name) IN ( ' || l_head || '))';
...
--  WITH q AS (
--           select * 
--             from (select distinct ts_name 
--                     from v_db_ts_alloc 
--                    where db_name = upper(p_db_name)
--                      and run_date = trunc(sysdate) 
--                      and ts_name not in ('SYSTEM','SYSAUX','TOOLS_AUTO_01','UNDOTBS','USERS_AUTO_01')
--                   order by allocated_gb desc) 
--            where rownum < 21 )
--    SELECT '|| Run Date || ' || 
--           LISTAGG ( ts_name  , ' || ' ) WITHIN GROUP ( ORDER BY ts_name ) ||
--           '||'
--      into l_head     
--      FROM q ;
...
--                   order by allocated_gb desc) 
--              where rownum < 21 )
--    SELECT LISTAGG ( ''''|| ts_name  , ''', ' ) WITHIN GROUP ( ORDER BY ts_name ) || '''' into l_head FROM q ;

--    select count(*) into l_cnt from user_tables where table_name = 'TS_SPACE_INFO';
--    if l_cnt = 1 then
--      execute immediate 'drop table TS_SPACE_INFO';
--    end if;
--    l_sql := 'create table TS_SPACE_INFO as with q as ( SELECT to_char(run_date,''dd/mm'') run_date, ts_name, allocated_gb FROM hp_diag.v_db_ts_alloc where db_name = ''' || upper(p_db_name) || ''' and run_date > add_months(trunc(sysdate),-3) group by run_date, ts_name, allocated_gb ) select *  from  q pivot (max(allocated_gb) for (ts_name) IN ( ' || l_head || ')) order by to_date(run_date,''dd/mm'')';
--    execute immediate l_sql;
    
--    with q as (select column_id, column_name from user_Tab_columns where table_name = 'TS_SPACE_INFO' order by column_id)
--           SELECT LISTAGG ( '|| "'|| column_name  , '" || ''||''' ||'' ) WITHIN GROUP ( ORDER BY column_id ) || '"'
--             INTO l_head
--             FROM q ;

--    l_sql := 'select ''||'' ' || l_head || '|| '''||'||'' from TS_SPACE_INFO order by to_date(run_date, ''dd/mm'')';
--    execute immediate l_sql bulk collect into l_info;
--    FOR indx IN 1 .. l_info.COUNT
--    LOOP
--      DBMS_OUTPUT.put_line (l_info(indx).row_data);
...
BEGIN
  ...
  dbms_output.put_line('{toc:type=list}');
  dbms_output.put_line('h5. Overview') ;
