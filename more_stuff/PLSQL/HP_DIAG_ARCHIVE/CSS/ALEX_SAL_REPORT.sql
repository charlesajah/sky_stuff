CREATE OR REPLACE PACKAGE         alex_sal_report
/* N02
|| Name : hp_diag.report
...
AS
   ...
   FUNCTION load_comparison_single_db (
        i_testId1 IN hp_diag.test_result_metrics.test_id%TYPE DEFAULT 'Latest test',
        i_db_name IN hp_diag.test_result_metrics.database_name%type
      ) RETURN g_tvc2 PIPELINED ;
...
CREATE OR REPLACE PACKAGE BODY         alex_sal_report
/* N02
|| Name : hp_diag.report
|| Database : chordo (N01) and ccs021n (N02)
|| Usage : SELECT * FROM TABLE ( hp_diag.report.chart_comparison )
...
BEGIN
   ...
   LOOP
      ...
      PIPE ROW ( '{chart:type=bar | width=450 | height=450 | title = ' || l_metric_name ( l_idx ) || ' | orientation = horizontal}' ) ;
        WITH q AS (
         SELECT DISTINCT database_name
           FROM hp_diag.test_result_metrics
          WHERE metric_name = l_metric_name ( l_idx )
            AND test_id IN ( l_testId1 , l_testId2 )
            and database_name = 'ISS021N'
         )
   ...
   FOR rh IN (
      ...
      select distinct database_name from TEST_RESULT_WAIT_CLASS t where t.test_id IN ( l_testId1 ) and database_name = 'ISS021N' order by 1
   )
   LOOP
      PIPE ROW ( '{chart:type=bar | width=500 | height=500 | title = Average Session Breakdown for ' || rh.database_name || ' | orientation = horizontal}' ) ;
...
BEGIN
   ...
   PIPE ROW ( 'h5. Top 15 SQL Comparison Across Database by Elapsed Time' ) ;
      ...
      WITH cur AS (
         SELECT t.database_name
          ...
          WHERE t.test_id = l_testId1
            ...
            AND t.database_name IN ('ISS021N')
   ...
   LOOP
      PIPE ROW ( r1.col1 ) ;
   END LOOP ;
   PIPE ROW ( 'h5. Top 15 SQL Comparison Across Database by Elapsed Time (with values)' ) ;
   -- end of 17-Apr-2023 RAG additions
   PIPE ROW ( '{csv:allowExport=true|columnTypes=s,i,i,s,f,f,f,f,s,s|id=Top15Comparison}' ) ;
   FOR rh IN (
      SELECT MIN ( CASE m.test_id WHEN l_testId1 THEN m.test_description END ) AS test_description1
           , MIN ( CASE m.test_id WHEN l_testId2 THEN m.test_description END ) AS test_description2
        FROM hp_diag.test_result_master m
   ...
   LOOP
      PIPE ROW ( '"SQL ID","Position","' || NVL ( rh.test_description2, l_testid2 ) || ' Position","Database Name","TPS","' || NVL ( rh.test_description2 , l_testid2 ) || ' TPS","ms Per Exec","' || NVL ( rh.test_description2 , l_testid2 ) || ' ms Per Exec","SQL Text","Module"');
   END LOOP ;
...
BEGIN
   ...
   --l_row := 'h3. Top 15 SQL Comparison Across Database by Previous Releases' ;
   --PIPE ROW ( l_row ) ;
...
BEGIN
   ...
   l_row := 'h5. Top 15 SQL Comparison Across Database by Elapsed Time' ;
   PIPE ROW ( l_row ) ;
   PIPE ROW ( '{csv:allowExport=true|columnTypes=s,s,f,f,f,f,f,f,f,f,s|id=Top15Comparison}' ) ;
...
BEGIN
   ...
   l_row := 'h5. Top 15 Row Lock Waits Across Database by Total Waited Time' ;
   PIPE ROW ( l_row ) ;

   PIPE ROW ( '{csv:allowExport=true|columnTypes=s,s,s,i,s,f,f,f,f,f,f,f,f,f,f|id=Top15RLWComparison}' ) ;
   FOR rh IN (
      SELECT MIN ( CASE m.test_id WHEN l_testId1 THEN m.test_description END ) AS test_description1
           , MIN ( CASE m.test_id WHEN l_testId2 THEN m.test_description END ) AS test_description2
        FROM hp_diag.test_result_master m
...
BEGIN
   ...
   PIPE ROW ( '{csv:allowExport=true|columnTypes=s,s,s,s,s,s,s,s,s,s,s,s|id=Top15Comparison}' ) ;
...
BEGIN
   ...
   l_row := 'h5. Top ' || i_limit || ' SQL Comparison for ' || i_dbname || ' by Elapsed Time' ;
   PIPE ROW ( l_row ) ;
